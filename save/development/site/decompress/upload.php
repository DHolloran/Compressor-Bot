...
class Upload {
	...
	public function __construct()
	{
		...
		$this->_msg 	 = array();
		$this->_path 	 = UploadConfig::$baseDir.'/'.UploadConfig::$relativefolderPath;
	}
	...
	public function _update($array,$id){
		$this->_database=new UploadDatabase();
		$this->_database->connect();
		return $this->_database->update('files',$array,array('id=',intval($id)));
	}
	...
	public function debug()
	{
		...
		if(ini_get('safe_mode')) {
			$msg[]='Error: PHP is running on safe_mode, you need to at least disable safe_mode for dir '.$this->_path;
		}
	...
	public function upload($file)
	{
		...
		if( sizeof($this->_allowExt)>0 && (!in_array(strtolower($fileExt),$this->_allowExt)) ){
			$this->_setMsg('incorrect file ext');
			return false;
		}
		//check destination folder permission
		if( !is_writable(dirname($this->_path))) {
			$thsi->setMsg($this->_path.' is not writable');
			return false;
		}
		//insert into database
		if($this->_insert(array('name'=>$fileName,'size'=>$fileSize,'ext'=>$fileExt))){
			//current insert value
			$current = $this->_readAll();
			$current = $current[0];
			//dir for storing the file
			$dir  = $this->_path.'/'.$current['id'];
			$path = UploadConfig::$relativefolderPath.'/'.$current['id'].'/'.$fileName;
			//create dir
	...
	public function delete($id)
	{
		...
		//check file exsit
		if(!file_exists(UploadConfig::$baseDir.'/'.$file['path'])){
			$this->_setMsg('file not found');
			return false;
		};
		//check file writable
		if(!is_writable(UploadConfig::$baseDir.'/'.$file['path'])){
			$this->_setMsg($file['name'].' is not writable');
			return false;
		};
		//check folder writable
		if(!is_writable(dirname(UploadConfig::$baseDir.'/'.$file['path']))){
			$this->_setMsg($file['name'].' container folder is not writable');
			return false;
		};
		//remove folder
		if($this->_removeFolder(dirname(UploadConfig::$baseDir.'/'.$file['path']))){
			if($this->_delete($id)){
				return true;
			}else{
	...
	public function deleteAll()
	{
		...
		if(!is_writable((UploadConfig::$baseDir.'/'.UploadConfig::$relativefolderPath))){
			$this->_setMsg(UploadConfig::$baseDir.'/'.UploadConfig::$relativefolderPath.' folder is not writable');
			return false;
		};
